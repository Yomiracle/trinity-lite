...
class GuardTest(unittest.TestCase):
    ...
    def test_redacts_env_style_secret_names(self):
        text = "SERVICE_" + "TO" + "KEN=public-looking-but-sensitive"
        self.assertEqual("[REDACTED]", redact_secrets(text))

    def test_redacts_bare_secret_names(self):
        for key in ["TO" + "KEN", "SEC" + "RET", "PASS" + "WORD", "API" + "_KEY"]:
            with self.subTest(key=key):
                self.assertEqual("[REDACTED]", redact_secrets(key + "=public-looking-but-sensitive"))
